...
void registry::load_from_scene(const MG1::Scene &scene) {
  ...
  for (const auto &sc0 : scene.surfacesC0) {
    std::vector<ecs::EntityType> points((sc0.size.x * 3 + 1) *
                                        (sc0.size.y * 3 + 1));
    for (uint32_t ypat = 0; ypat < sc0.size.y; ++ypat) {
      for (uint32_t xpat = 0; xpat < sc0.size.x; ++xpat) {
        // build points
        auto &cp = sc0.patches[ypat * sc0.size.x + xpat];
        for (int jj = ypat == 0 ? 0 : 1; jj < 4; ++jj) {
          for (int ii = xpat == 0 ? 0 : 1; ii < 4; ++ii) {
              const auto global_x = xpat * 3 + ii;
              const auto global_y = ypat * 3 + jj;
            points[global_y * (sc0.size.x * 3 + 1) + global_x] =
    ...
    constructors::add_bezier_surface(points, patches);
  }
  // surface C2
  for (const auto &sc2 : scene.surfacesC2) {
    std::vector<ecs::EntityType> points((4 + sc2.size.x - 1) *
                                        (4 + sc2.size.y - 1));
    for (uint32_t ypat = 0; ypat < sc2.size.y; ++ypat) {
      for (uint32_t xpat = 0; xpat < sc2.size.x; ++xpat) {
        // build points
        auto &cp = sc2.patches[ypat * sc2.size.x + xpat];
        for (int jj = ypat == 0 ? 0 : 3; jj < 4; ++jj) {
          for (int ii = xpat == 0 ? 0 : 3; ii < 4; ++ii) {
              const auto global_x = (xpat == 0) ? ( ii ) : (4 + xpat - 1);
              const auto global_y = (ypat == 0) ? ( jj ) :( 4 + ypat - 1);

              points[global_y * (sc2.size.x + 4 - 1) + global_x] =
                  idx_scene_to_ecs[cp.controlPoints[jj * 4 + ii].GetId()];
            }
        }
      }
    ...
    constructors::add_bspline_surface(points, patches);
  }
}
