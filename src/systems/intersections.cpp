...
inline bool needs_wrapping(float v) {
  return v < 0.f || v > 1.f;
}
...
inline intersection_status extend_start_point(std::vector<glm::vec3>& points, std::vector<glm::vec4>& coords, const glm::vec4& start_coords, 
                               float delta, sampler& first, sampler& second,
...
intersection_status find_all_intersection_points(const intersection_params& params, glm::vec4 start_coords,
  ...
  auto extension_status = extend_start_point(points, coords, start_coords, params.delta, first, second, params);
  ...
  if(!cycle_found) {
    extend_start_point(tmp_points, tmp_coords, start_coords, -params.delta, first, second, params);
  }
...
intersection_status intersect(ecs::EntityType first_idx, ecs::EntityType second_idx, sampler &first, sampler &second, 
  ...
  auto newton_status = find_all_intersection_points(params, start_coords, points, coords, first, second);
  ...
  if(!self_intersection) {
    ...
    float clear = 0.f;
    glClearTexSubImage(tex[0], 0, 0, 0, 0, texture_size, texture_size, 1, GL_RED, GL_FLOAT, &clear);
    glClearTexSubImage(tex[1], 0, 0, 0, 0, texture_size, texture_size, 1, GL_RED, GL_FLOAT, &clear);
    ...
    for(std::size_t seg = 1; seg < coords.size(); ++seg) {
      ...
      auto first_c = glm::vec4{wrap(fc1.x), wrap(fc1.y), wrap(fc1.z), wrap(fc1.w)};
      auto second_c = coords[seg];

      Bresenham<texture_size>(first_c.x, first_c.y, second_c.x, second_c.y, set_pixel);
      if(needs_wrapping(second_c.x) || needs_wrapping(second_c.y)) {
        if(second_c.x < 0.f) {
          Bresenham<texture_size>(first_c.x + 1.f, first_c.y, second_c.x + 1.f, second_c.y, set_pixel);
        } else if(second_c.x > 1.f) {
          Bresenham<texture_size>(first_c.x - 1.f, first_c.y, second_c.x - 1.f, second_c.y, set_pixel);
        }

        if(second_c.y < 0.f) {
          Bresenham<texture_size>(first_c.x, first_c.y + 1.f, second_c.x, second_c.y + 1.f, set_pixel);
    ...
    for(std::size_t seg = 1; seg < coords.size(); ++seg) {
      ...
      Bresenham<texture_size>(first_c.z, first_c.w, second_c.z, second_c.w, set_pixel);
      if(needs_wrapping(second_c.z) || needs_wrapping(second_c.w)) {
        if(second_c.z < 0.f) {
          Bresenham<texture_size>(first_c.z + 1.f, first_c.w, second_c.z + 1.f, second_c.w, set_pixel);
        } else if(second_c.z > 1.f) {
          Bresenham<texture_size>(first_c.z - 1.f, first_c.w, second_c.z - 1.f, second_c.w, set_pixel);
        }

        if(second_c.w < 0.f) {
          Bresenham<texture_size>(first_c.z, first_c.w + 1.f, second_c.z, second_c.w + 1.f, set_pixel);
